...
export default function MonthlyReservationScreen() {
  ...
  // 🛠️ ปรับแก้ข้อความต้อนรับและหัวข้อจาก รายวัน -> รายเดือน เรียบร้อยแล้วครับ!
  const text = {
    TH: {
      welcomeGuest: 'Around Loei (รายเดือน)',
      welcomeUser: 'สวัสดีคุณ ',
      location: 'ในเมืองเลย ใกล้ มรภ.เลย',
      selectTitle: 'ห้องพักว่างสำหรับเริ่มสัญญา',
      floor: 'ชั้นที่ ', available: 'ว่างพร้อมทำสัญญา', backHome: 'เปลี่ยนวันที่เริ่มเช่า',
      unitM: ' /เดือน', confirmBtn: 'ยืนยันการจองทำสัญญารายเดือน',
      loginRequiredBtn: 'กรุณาเข้าสู่ระบบก่อนทำการจองห้องพัก',
      success: 'ส่งคำขอจองสำเร็จ!', viewList: 'ดูรายการจองของคุณ',
      desc: 'สัมผัสประสบการณ์การพักผ่อนระดับพรีเมียม สไตล์มินิมอล พร้อมระบบความปลอดภัย 24 ชม. สำหรับการเข้าพักระยะยาวรายเดือน',
      fail: 'ไม่สามารถจองได้',
      pickDateMonthly: 'ระบุวันที่ต้องการเริ่มเข้าพัก (สัญญาขั้นต่ำ 1 เดือน)',
      goBack: 'กลับสู่หน้าหลัก',
      home: 'กลับหน้าหลัก',
      logout: 'ออกจากระบบ'
    },
    EN: {
      welcomeGuest: 'Around Loei (Monthly)',
      welcomeUser: 'Welcome, ',
      location: 'Loei City, near LRU',
      selectTitle: 'Available for Move-in on',
      floor: 'Floor ', available: 'Vacant', backHome: 'Change Move-in Date',
      unitM: ' /month', confirmBtn: 'Confirm Long-term Contract',
      loginRequiredBtn: 'Please Login to Continue Booking',
      desc: 'Experience premium living with minimalist style and 24h security. Designed for comfortable long-term monthly stays.',
      fail: 'Booking failed',
      pickDateMonthly: 'Select Move-in Date (1 Month Minimum)',
      goBack: 'Back to Home',
      home: 'Back to Home',
      logout: 'Logout'
    }
  ...
  >
    ...
    <View style={{
      ...
      <Modal visible={showInitialModal} transparent animationType="fade">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.75)', justifyContent: 'center', padding: 20 }}>
          <View style={{ backgroundColor: 'white', borderRadius: 30, padding: 25, maxHeight: '90%' }}>
            ...
            <Text style={{ fontSize: 20, fontWeight: '800', color: '#1E293B', textAlign: 'center', marginBottom: 20, paddingHorizontal: 15 }}>{t.pickDateMonthly}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={{ marginTop: 10, borderRadius: 20, overflow: 'hidden', borderWidth: 1, borderColor: '#E2E8F0' }}>
                <Calendar
                  ...
                  onDayPress={day => {
                    ...
                    const currentStart = new Date(day.timestamp);
                    const nextMonth = new Date(currentStart.setMonth(currentStart.getMonth() + 1));
              ...
              <View style={{ flexDirection: 'row', backgroundColor: '#F1F5F9', borderRadius: 20, padding: 18, borderWidth: 1, borderColor: '#E2E8F0', alignItems: 'center', opacity: 0.85 }}>
                <View style={{ backgroundColor: '#E2E8F0', padding: 8, borderRadius: 12, marginRight: 15 }}>
                  <Ionicons name="time-sharp" size={24} color="#64748B" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 11, color: '#94A3B8', fontWeight: '900' }}>END OF CONTRACT (1 MONTH MINIMUM)</Text>
                  <Text style={{ fontSize: 18, color: '#64748B', fontWeight: '700', marginTop: 2 }}>{formatDateTH(endDate)}</Text>
                </View>
              </View>
              <TouchableOpacity onPress={handleConfirmInitialDate} style={{ backgroundColor: '#0194F3', marginTop: 25, paddingVertical: 18, borderRadius: 25, alignItems: 'center', elevation: 5 }}>
                <Text style={{ color: 'white', fontWeight: '900', fontSize: 18 }}>ค้นหาห้องว่างสำหรับรายเดือน</Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        </View>
      ...
      <Animated.View style={{
        ...
        <TouchableOpacity onPress={() => hideNotification(() => router.push('/(monthly)/reservationlist'))} style={{ backgroundColor: '#0194F3', paddingHorizontal: 18, paddingVertical: 10, borderRadius: 14 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#0194F3']} />}>
        <View style={{ height: 220, width: '100%', position: 'relative' }}>
          ...
          <View style={{ position: 'absolute', bottom: 30, left: 25 }}>
            <Text style={{ color: 'white', fontSize: 32, fontWeight: '900' }}>Around Loei รายเดือน</Text>
        ...
        <View style={{ padding: 25 }}>
          <View style={{ marginBottom: 20 }}>
            ...
            <Text style={{ fontSize: 15, color: '#0194F3', fontWeight: '700', marginTop: 4 }}>
              เริ่มเช่า: {formatDateTH(startDate)} (สิ้นสุดสัญญารอบแรก: {formatDateTH(endDate)})
            </Text>
          </View>
          ...
          {fetching ? (
            ...
            floors.length > 0 ? (
              ...
              <View style={{ alignItems: 'center', marginTop: 20 }}>
                <Text style={{ color: '#94A3B8' }}>ไม่พบห้องสำหรับทำสัญญารายเดือนในช่วงเวลานี้</Text>
              </View>
            )
          )}
              ...
              <View style={{ padding: 25 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <View>
                    ...
                    <Text style={{ color: '#94A3B8', marginTop: 4, fontSize: 14, fontWeight: '600' }}>รายเดือนสุดพรีเมียม</Text>
                  </View>
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={{ fontSize: 26, fontWeight: '900', color: '#0194F3' }}>฿{selectedRoom?.basePriceMonthly}</Text>
                    <Text style={{ fontSize: 12, color: '#64748B', fontWeight: 'bold' }}>{t.unitM}</Text>
                  </View>
                </View>
                <View style={{ backgroundColor: '#F8FAFC', borderRadius: 20, padding: 18, borderWidth: 1, borderColor: '#E2E8F0' }}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 }}>
                    <Text style={{ color: '#94A3B8' }}>วันที่เริ่มสัญญา:</Text>
                    <Text style={{ fontWeight: '700', color: '#1E293B' }}>{formatDateTH(startDate)}</Text>
                  </View>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ color: '#94A3B8' }}>วันที่สิ้นสุดสัญญารอบแรก:</Text>
                ...
                {user ? (
                  <TouchableOpacity disabled={loading} onPress={handleConfirmBooking} style={{ backgroundColor: '#0194F3', paddingVertical: 18, borderRadius: 22, alignItems: 'center', marginTop: 30, marginBottom: 40, elevation: 5 }}>
                  ...
                  <TouchableOpacity onPress={() => { setSelectedRoom(null); router.push('/(auth)/login'); }} style={{ backgroundColor: '#FF7043', paddingVertical: 18, borderRadius: 22, alignItems: 'center', marginTop: 30, marginBottom: 40 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
